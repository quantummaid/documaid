...
class ArchetypeMarkdownHandler : MarkdownTagHandler {

    override fun tag(): String = ARCHETYPE_TAG.value
    ...
    override fun generate(
        ...
        val archetypeDirective = ArchetypeDirective.create(directive, file, project)
        val textToReplace = archetypeDirective.generateCompleteMarkdown()
    ...
    override fun validate(
        ...
        return if (textToBeReplaced != textToReplace) {
            ...
            if (trailingCodeFound) {
                val message = "Found [${tag()}] tag with incorrect archetype code for '${directive.completeString}'"
                listOf(VerificationError.create(message, file))
            } else {
                val message = "Found [${tag()}] tag with missing archetype code for '${directive.completeString}'"
                listOf(VerificationError.create(message, file))
            }
        } else {
            emptyList()
    ...
    private fun textToBeReplaced(directive: RawMarkdownDirective): Pair<String, TrailingMarkdownMatchResult> {
        ...
        val trailingMarkdownMatchResult = ArchetypeMarkdown.startsWithArchetypeMarkdown(remainingContent)
